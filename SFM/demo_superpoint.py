...
class SuperPointFrontend(object):
  ...
  def run(self, img):
    ...
    Output
      pnts - 3xN numpy array with corners [x_i, y_i, confidence_i]^T.
...
if __name__ == '__main__':
  ...
  while True:
    ...
    # get the current image name from vs object, add suffix for output file
    mat_file_name = vs.listing[vs.i-1][:-4] + "_superpoint.mat"
    print("Saving feature data in .mat format to: " + mat_file_name)
    # feature array combined by postions (pts[:2, :]) and descriptor (desc)
    feature = np.vstack((pts[:2, :], desc))
    # save to Matlab file, i.e. 000abc__superpoint.mat
    # note to convert the np.array to sigle precision (np.float32 in Python)
    sio.savemat(mat_file_name, {'feature': np.array(feature, dtype=np.float32)})
